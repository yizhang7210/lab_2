...
county_populations <- read.csv('../data/raw/county-population.csv')
county_populations <- county_populations %>%
  select(FIPStxt, POP_ESTIMATE_2019) %>%
  mutate(population = as.numeric(gsub(",", "", POP_ESTIMATE_2019)))
county_populations$COUNTYFP <- county_populations$FIPStxt

mask_use <- mask_use %>% mutate(
  mask_prob = ALWAYS + FREQUENTLY * 0.8 + SOMETIMES * 0.5 + RARELY * 0.2,
  fips = as.integer(COUNTYFP/1000),
) %>% inner_join(county_populations, by=c("COUNTYFP"))
mask_use <- mask_use %>% mutate(
  masked_population = mask_prob * population
)

mask_use_states <- mask_use %>% group_by(fips) %>% summarise(
  mask_prob = sum(masked_population) / sum(population),
)
...
data <- data %>% select(state, cases_per_100k, workplaces, transit, grocery, popden,
                        parks, residential, stay_home, fm, mask_prob, mask_days)
```
...
including to workplaces, transit, grocery & pharmacy.
...
p2 <- ggplot(data, aes(x=residential, y=cases_per_100k)) + 
  geom_point(color="#69b3a2") + geom_smooth(se = FALSE)
p3 <- ggplot(data, aes(x=transit, y=cases_per_100k)) + 
  geom_point(color="#69b3a2") + geom_smooth(se = FALSE)
...
model_two <- lm(cases_per_100k ~ residential + log(popden) + mask_prob, data=data)
summary(model_two)
```
...
Our definitive third model is the comparison of all the mobility score variables,
...
model_three <- lm(cases_per_100k ~ log(popden)  + mask_prob + residential + parks + grocery + transit, data=data)
summary(model_three)
```
...
stargazer::stargazer(model_one, model_two, model_three, 
  title = 'Comparisoin of Regression Models')
```
