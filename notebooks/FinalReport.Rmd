...
data <- data %>% select(state, cases_per_100k, workplaces, transit, grocery, popden,
                        pop, parks, residential, stay_home, fm, mask_prob, mask_days)
```
...
model_two <- lm(cases_per_100k ~ residential + fm + mask_prob , data=data)
summary(model_two)
```
...
model_three <- lm(cases_per_100k ~ residential + fm + mask_prob + parks + grocery + transit, data=data)
summary(model_three)
```
