...
p1 <- ggplot(california, aes(x=date, y=cases)) + geom_line(color="#69b3a2") + xlab("")
...
```{r cal_residuals}
model_one_cal <- lm(rate_cases ~ residential_ra7, data=california_weekly)
california_weekly %>% 
  mutate(
...
# model_two <- lm(delta7_cases ~ residential_ra7 + popden + fm, data=weekly)
summary(model_two)
```
